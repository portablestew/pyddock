...
```json
{
  "mcpServers": {
    "pyddock": {
      ...
      "autoApprove": ["run_python","run_shell","fs_readfile","fs_stat","fs_find","fs_grep","fs_append","fs_delete","fs_str_replace"]
    }
  }
}
```
...
- **`fs_find`** — Find files by name (`file_glob` or `file_regex`, mutually exclusive). Prunes hidden dot-directories and `exclude_regex` matches before descending (never walked, not just filtered), so large ignored trees (`.venv/`, `node_modules/`) stay fast. Returns matching relative paths, capped at `max_results`.
- **`fs_grep`** — Search file contents by `grep_regex` (always case-insensitive). Same hidden-directory pruning, `exclude_regex`, and `file_glob`/`file_regex` file filtering as `fs_find`. Returns `path:line: content` per match with surrounding context lines (default: 1 for directory scans, 4 for single-file), capped at `max_results` and 300 chars per line, with an optional `max_results_per_file` cap. Skips binary files when scanning a directory; always searches a directly named file.
